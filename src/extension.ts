...
export function activate(context: vscode.ExtensionContext) {
    const newFileHandler = async function (e: vscode.Uri) {
        try {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('templateGenerator.newFile', newFileHandler),
    );
    context.subscriptions.push(
        vscode.commands.registerCommand('templateGenerator.newFileOadl', newFileHandler),
    );
